...
Water_KeywordToken_t* Water_GetKeywordToken_hash(uint32_t hash)
{
  return (Water_KeywordToken_t*)HashTable_keywords[hash];
}

Water_Var_t* Water_GetVar_hash(uint32_t hash)
{
  return (Water_Var_t*)HashTable_Vars[hash];
}

Water_OperatorObject_t* Water_GetOpObj_hash(uint32_t hash, uint32_t* HashTable)
{
  return (Water_OperatorObject_t*)HashTable[hash];
}

void Water_SetKeywordToken_hash(uint32_t hash, uintptr_t val)
{
  HashTable_keywords[hash] = val;
}

void Water_SetVar_hash(uint32_t hash, uintptr_t val)
{
  HashTable_Vars[hash] = val;
}

void Water_SetOpObj_hash(uint32_t hash, uintptr_t val, uint32_t* HashTable)
{
  HashTable[hash] = val;
}
...
{
  Water_KeywordToken_t* tmp = (Water_KeywordToken_t*)Water_alloc(sizeof(Water_KeywordToken_t));
...
uint32_t Water_GenTokenList(const char* str, uint32_t nToks, Water_Token_t* TokenStart)
{
  ...
  for(;nt < nToks;)
  {
    ...
    else if(tmpStr[0] == '(')   // A Round Bracket means another arithmatic/logical expression
    {
      ...
      --i;
      tmpStr[i] = '\0';
      token = (Water_Token_t*)Water_MakeTokenListSY(tmpStr, tmpToken);   //  Execute the Internal Expression RECURSIVELY
      ++tmpStr;
      if(!token)
      {
    ...
    else if(tmpStr[0] != ' ')   // Must be an Operator
    {
      ...
      printf("\n{%s}", tt);
      // Get the Token
      Water_Token_t* token = (Water_Token_t*)Water_GetOpToken(tmpStr, i, (uint32_t*)(((Water_BasicTypeToken_t*)tmpToken)->TypeClass->OperatorTable));   //  Execute the Internal Expression RECURSIVELY
...
void Create_BasicTypeClasses()
{
  ...
  Water_INT_TypeClass->OperatorTable = (uintptr_t)malloc(4096);
  uint32_t* tb = (uint32_t*)Water_INT_TypeClass->OperatorTable;
...
void Water_shell()
{
  ...
  while(1)
  {
    ...
    /******* MAIN PART ********/
    int error = Water_Execute((const char*)buf);
    /********** END **********/
...
int Water_MakeTokenList(const char* sentence, Water_Token_t* TokenStart)
{
  ...
  char* tmpStr = (char*)malloc(SentenceSize+2);
 ...
 // char tmp;
  ...
  for(;tmpStr[0] != '\0';)
  {
    ...
    else if(tmpStr[0] != ' ')   // Must be an Operator
    {
      ...
      {
        token = (Water_Token_t*)Water_GetOpToken(tmpStr, i, (uint32_t*)((Water_BasicTypeToken_t*)tmpToken)->TypeClass->OperatorTable);
      }
      else
      {
...
//  char tmp;
  ...
  for(;tmpStr[0] != '\0';)
  {
    ...
    {
      Water_Token_t* token;
      token = (Water_Token_t*)Water_GetOpToken(tmpStr, 1, (uint32_t*)((Water_BasicTypeToken_t*)tmpToken)->TypeClass->OperatorTable);   //  Execute the Internal Expression RECURSIVELY
      if(!token)
      {
        printf("\n%s Internal Expression Error", tmpStr);
        return 4;
...
Water_Token_t* Water_ArithmaticSolver(Water_Token_t* TokenStart)
{
  ...
  tree = (Water_OperatorToken_t*)Water_ReduceOpTree(tree);
...
Water_BasicTypeToken_t* Water_ReduceOpTree(Water_OperatorToken_t* treeStart)
{
  ...
  //printf("\n[%s]", treeStart->name);
  return (Water_BasicTypeToken_t*)(treeStart->op->Function((uintptr_t)treeStart));
}
...
Water_Token_t* Water_ShuntingYardSolver(Water_Token_t* TokenStart)
{
  ...
  for(;tmpToken->NextToken;)
  {
    if(tmpToken->type & WATER_OPERATOR)
    {
      ...
      {
      //  printf("-->Left Bracket");
        Water_PushOpStack((Water_OperatorToken_t*)tmpToken);
      }
...
Water_Token_t* Water_EvaluateShuntingYard()
{
  ...
  while(tmp)
  {
    ...
    {
      opTmp = (Water_OperatorToken_t*)tmp;
      //printf("\n====>%s", opTmp->name);
      opTmp->RHS = (Water_Token_t*)Water_PopValStack();
      opTmp->LHS = (Water_Token_t*)Water_PopValStack();
      Water_PushValStack((Water_BasicTypeToken_t*)(opTmp->op->Function((uintptr_t)opTmp)));
    }
    else
    {
    //  printf("\n<{%s}>", ((Water_BasicTypeObject_t*)tmp)->name);
      Water_PushValStack((Water_BasicTypeToken_t*)tmp);
    }
    tmp = Water_PopQueue();
  }
